-- =============================================================
-- Jyotirling Connect — Database Schema
-- Run this in your Supabase SQL Editor to create / migrate all tables.
...
-- Safely add new columns (idempotent for existing databases)
...
SET days_itinerary = jsonb_build_array(
  jsonb_build_object('day', 1, 'title', 'Day 1', 'description', '', 'notes', '', 'places',
    COALESCE(array_to_json(places_to_visit), '[]'::json))
)
WHERE jsonb_array_length(days_itinerary) = 0
  AND places_to_visit IS NOT NULL
  AND array_length(places_to_visit, 1) > 0;
...
-- ── 3. users (must exist before bookings for the FK) ─────────
...
CREATE TABLE IF NOT EXISTS public.bookings (
    ...
    -- User association (populated when the booker is signed-in)
...
-- Safely add new columns for Testimonial Submission Portal
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS email TEXT;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS phone TEXT;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS city TEXT;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS profile_image_url TEXT;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS gallery_images TEXT[];
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS package_name TEXT;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS fleet_name TEXT;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS travel_date DATE;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS overall_rating INTEGER;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS driver_rating INTEGER;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS vehicle_rating INTEGER;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS support_rating INTEGER;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS darshan_rating INTEGER;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS experience TEXT;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS recommendation TEXT;
ALTER TABLE public.testimonials ADD COLUMN IF NOT EXISTS consent_to_publish BOOLEAN DEFAULT false;
...
-- Data migrations for testimonials (mapping old fields to new fields)
UPDATE public.testimonials SET overall_rating = rating WHERE overall_rating IS NULL AND rating IS NOT NULL;
UPDATE public.testimonials SET experience = message WHERE experience IS NULL AND message IS NOT NULL;
UPDATE public.testimonials SET profile_image_url = image_url WHERE profile_image_url IS NULL AND image_url IS NOT NULL;
UPDATE public.testimonials SET city = location WHERE city IS NULL AND location IS NOT NULL;
...
CREATE POLICY "Public read access to testimonials"     ON public.testimonials     FOR SELECT USING (is_approved = true OR status = 'approved');

DROP POLICY IF EXISTS "Public insert access to testimonials"   ON public.testimonials;
CREATE POLICY "Public insert access to testimonials"   ON public.testimonials     FOR INSERT WITH CHECK (true);
...
-- Public can insert bookings (guest checkout + signed-in)
...
-- Users table: public access needed for NextAuth sign-in / profile read
DROP POLICY IF EXISTS "Public access to users for auth"        ON public.users;
CREATE POLICY "Public access to users for auth"        ON public.users            FOR ALL USING (true);
...
SELECT
  'Jyotirling Connect',
  'Premium Pilgrimage Travel',
  'Premium devotional travel agency serving pilgrims on their journey between Mahakaleshwar and Omkareshwar.',
  '© 2024 Jyotirling Connect. All rights reserved.'
...
SELECT
  ARRAY['+91 98765 43210'],
  'info@jyotirlingconnect.com',
  'Near Mahakaleshwar Temple, Ujjain, Madhya Pradesh 456006',
  'Monday - Sunday
9:00 AM - 8:00 PM'
